...
def generate_wrapper(sch_db, wrapper_params, cv_info_list, dut_netlist, gen_sch):
...
def run_main(prj: BagProject, args: argparse.Namespace) -> None:
    ...
    cv_info = []
    dut_netlist = prj.generate_cell(specs['dut_params'], lay_db=lay_db, sch_db=sch_db,
                                    gen_lay=args.gen_lay, gen_sch=args.gen_sch,
                                    cv_info_out=cv_info,
                                    run_rcx=args.run_rcx)

    print(f'dut_netlist: {dut_netlist}')
    generate_wrapper(sch_db, specs['wrapper_params'], cv_info, dut_netlist, gen_sch=args.gen_sch)
