...
class bag3_digital__DynOffCompSB_coreP_split(Module):
    """Module for library bag3_digital cell DynOffCompSB_coreP_split.
    ...
    def design(self, seg_dict: Dict[str, int], lch: int, w_dict: Dict[str, int],
        ...
        # nch devices + nch switches
        for _name, _str in zip(['XM1', 'XM2', 'XS1', 'XS2', 'XS3', 'XS4'],
                               ['gm2n', 'gm2n', 'sw12', 'sw12', 'sw34', 'sw34']):
            self.design_transistor(_name, w_dict[_str], lch, seg_dict[_str], th_dict[_str], m='')

        # pch devices
        for _name, _str in zip(['XM3', 'XM4', 'XMc1', 'XMc2'],
                               ['gm2p', 'gm2p', 'gm1', 'gm1']):
            self.design_transistor(_name, w_dict[_str], lch, seg_dict[_str], th_dict[_str], m='')
