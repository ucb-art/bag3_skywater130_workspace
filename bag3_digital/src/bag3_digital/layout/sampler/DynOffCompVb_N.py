"""This module contains layout generators for a DOC comparator with external Vb, NMOS input and
NMOS switches."""
...
class DynOffCompVb_coreN(MOSBase):
    """Core of DOC comparator with external Vb, NMOS input and NMOS switches.

    Assumes:

    1. 3 rows: nch (flipped), nch (flipped), pch.
    ...
    def draw_layout(self) -> None:
        ...
        seg_tot = max((seg_gm2n + seg_gm1) * 2, (seg_sw34 + seg_sw12 + seg_sep) * 2 + seg_sw5,
                      (seg_gm2p + seg_en12 + seg_sep) * 2) + 2 * (sub_sep + tap_n_cols)
        ...
        gm2n_1 = self.add_mos(ridx_n, seg_tot2, seg_gm2n, abut=True, w=w_n)

        # input nmos
        gm1n_2 = self.add_mos(ridx_n, seg_tot2 - seg_gm2n, seg_gm1, w=w_n, flip_lr=True, abut=True)
        gm1n_1 = self.add_mos(ridx_n, seg_tot2 + seg_gm2n, seg_gm1, w=w_n, abut=True)

        _row_info = pinfo.get_row_place_info(ridx_n).row_info
        w_gm2n, th_gm2n = _row_info.width, _row_info.threshold

        # nch row sw12 + sw34 + sw5: break sw5 into 2 halves for symmetry
        if seg_sw5 % 2 != 0:
        ...
        sw5_right = self.add_mos(ridx_nsw, seg_tot2 + 1, seg_sw5_2, w=w_n, abut=True)
        sw5_left = self.add_mos(ridx_nsw, seg_tot2 - 1, seg_sw5_2, w=w_n, abut=True, flip_lr=True)
        ...
        # pch row for gm2p cells and enable switches
        ...
        self.connect_differential_tracks(gm2n_2.g[0:num_g_min], gm2n_1.g[0:num_g_min],
                                         hm_layer, sw5_l_idx, sw5_r_idx)
        ...
        self.add_pin('VOP_top', vop_top, label='VOP:', show=show_pins)
        self.add_pin('VOM_top', vom_top, label='VOM:', show=show_pins)
        self.add_pin('VOP_bot', vop_bot, label='VOP:', show=show_pins)
        self.add_pin('VOM_bot', vom_bot, label='VOM:', show=show_pins)
        self.add_pin('VOP_sw', sw1_term, label='VOP:', show=show_pins)
        self.add_pin('VOM_sw', sw2_term, label='VOM:', show=show_pins)

        # # --- Pins --- #
        self.add_pin('RST', clk, show=show_pins)
        self.add_pin('RSTD', clkd, show=show_pins)
        self.add_pin('VDD', vdd, label='VDD:', show=show_pins)
        self.add_pin('RX_EN', rx_en, show=show_pins)
...
class DynOffCompVb_Wrapper(MOSBaseWrapper):
    ...
    @classmethod
    def get_params_info(cls) -> Dict[str, str]:
        return DynOffCompVb_CapN.get_params_info()
    ...
    def draw_layout(self) -> None:
        ...
        master = self.new_template(DynOffCompVb_CapN, params=core_params)

        inst = self.draw_boundaries(master, master.top_layer, half_blk_x=False)
...
class DynOffCompVb_CapN(MOSBase):
    ...
    def draw_layout(self) -> None:
        ...
        doc_master = self.new_template(DynOffCompVb_coreN, params=doc_params)
        ...
        for _cap, _sw in zip([cap_cross_left, cap_cross_right], [sw_m, sw_p]):
            ...
            _plus = _cap.get_pin('plus', layer=xm_layer)
            _name = 'VOM_cap' if _cap == cap_cross_left else 'VOP_cap'
            _label = 'VOM:' if _cap == cap_cross_left else 'VOP:'
            self.add_pin(_name, _plus, label=_label)

        # re-export pins
        for name in ['RST', 'RSTD', 'Vb', 'VDD', 'VSS', 'RX_EN', 'VOP_top', 'VOM_top', 'VOP_sw',
                     'VOM_sw', 'VOP_bot', 'VOM_bot']:
